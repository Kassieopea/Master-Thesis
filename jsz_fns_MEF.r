...
getLambdaRVAR <- function(gamma, cP, K0Q.cP, K1Q.cP, OmegaInv, crossterm = NA) {
    ...
    if (missing(crossterm)){
      crossterm <- diag(N)
    } 
    ## restriction:  beta = R*lambda + r
    R <- matrix(0, (N+N*(2+N)), sum(gamma==1))
    # browser()
    R[ cbind(which(gamma==1), seq(1, sum(gamma))) ] <- 1
    r <- c(K0Q.cP, as.numeric(K1Q.cP + crossterm))
    ...
    z <- y - kronecker(t(Z), t(crossterm)) %*% r
    inv.cov.mat <- t(R) %*% kronecker(tcrossprod(Z), OmegaInv[1:N,1:N]) %*% R
    #browser()
    if (det(inv.cov.mat) > 10^10){
      # browser()
      cov.mat <- pseudoinverse(inv.cov.mat)
    } else if (det(inv.cov.mat) < 10^(-5)){
      cov.mat <- pseudoinverse(inv.cov.mat)
    ...
    lambda.hat <- as.numeric( cov.mat %*% t(R) %*% kronecker(Z, OmegaInv[1:N,]) %*% z ) 
    lambda.full <- R %*% lambda.hat
    return(list(lambda.hat=lambda.hat, Lam0=lambda.full[1:N], Lam1=matrix(tail(lambda.full, -N), N, (N+2)), cov.mat=cov.mat, inv.cov.mat=inv.cov.mat, R=R))
}

jsz.llk <- function (yields.o, W, ZP, K1Q.X, kinfQ=NA, K0P.cP=NA, K1P.cP=NA, Sigma.cQ = NA, Sigma.cP, mats, dt, sigma.e=NA, restr=0, ind.restr=NA, Lam1.rank=NA, ev.max=NA, Lam0, Lam1) {
...
########################################################################
    ...
    N <- nrow(W)
    Z_N <- ncol(ZP)
    cQ <- yields.o %*% t(W) # (T+1)*N, cP stands for math caligraphic P.
########################################################################
...
########################################################################
    ...
    if (is.na(kinfQ)) {
        ...
        #browser()
        loads <- jsz.loadings.rho0cP(W, K1Q.X, rho0.cP, Sigma.cQ, mats, dt)
        ...
        require(MASS)
        V <- t(Null(t(W)))
        kinfQ <- t(colMeans(yields.o[2:(T+1),]) - t(alpha1.cP) - t(BcP)%*%colMeans(cQ[2:(T+1),]))%*%(t(V)%*%V%*%t(alpha0.cP)) / (alpha0.cP%*%t(V)%*%V%*%t(alpha0.cP))
        ...
        loads <- jsz.loadings(W, K1Q.X, kinfQ, Sigma.cQ, mats, dt)
    ...
    yields.m <- rep(1,T+1)%*%AcP + cQ %*% BcP # (T+1)*J, model-implied yields
...
########################################################################
    ...
    if (any(is.na(K0P.cP))||any(is.na(K1P.cP))) {
        ...
        var1 <- ar.ols(ZP, order=1, aic=FALSE, demean=TRUE, intercept=TRUE)
        K1P.ZP <- var1$ar[,,] - diag(Z_N)
        # K1P.ZP <- K1P.ZP[1:N,]
        K0P.ZP <- var1$x.intercept
        # browser()
        # K0P.ZP <- K0P.ZP[1:N]
###########################
        if (restr==1) {
            ...
            if (missing(Lam0)|missing(Lam1)) {
                ...
                if (sum(ind.restr)>0) {
                    ...
                    mats <- getLambdaRVAR(ind.restr, ZP, K0Q.cP, cbind(K1Q.cP, matrix(rep(0,(2*N)), nrow = N)), pseudoinverse(Sigma.cP), crossterm)
                    # browser()
                    Lam0 <- mats$Lam0
                    Lam1 <- mats$Lam1
                } else {
            ...
            K0P.ZP[1:N] <- K0Q.cP + Lam0
            K1P.ZP[1:N,] <- cbind(K1Q.cP, matrix(rep(0, N*2), nrow = N)) + Lam1
            #browser()
        }
            ###########################
        # } else if (restr != 0) {
    ...
    # browser()
    innovations = t(cQ[2:(T+1),]) - (K0P.ZP[1:N]%*%matrix(1,1,T) + (K1P.ZP[1:N,]+cbind(diag(N), matrix(rep(0, (2*N)), nrow = N)))%*%t(ZP[1:T,])) # N*T

    llkP = .5*N*log(2*pi) + .5*log(det(Sigma.cQ)) + .5*colSums(innovations*solve(Sigma.cQ, innovations)) # 1*T
    # llkP_test = .5*Z_N*log(2*pi) + .5*log(det(Sigma.cP)) + .5*colSums(innovations*ginv(Sigma.cP) %*% innovations)
    # browser()
    ########################################################################
    # print("JSZ LOADINGS!!!")
    # print(sum(llkQ))
    # print(sum(llkP))
    jsz.llk <- list(llk=t(llkQ + llkP), AcP=AcP, BcP=BcP, AX=AX, BX=BX, K0P.cP=K0P.ZP, K1P.cP=K1P.ZP, sigma.e=sigma.e, K0Q.cP=K0Q.cP, K1Q.cP=K1Q.cP, rho0.cP=rho0.cP, rho1.cP=rho1.cP, K0Q.X=K0Q.X, K1Q.X=K1Q.X, cP=ZP, llkP=llkP, llkQ=llkQ)
...
jsz.loadings <- function(W, K1Q.X, kinfQ, Sigma.cP, mats, dt) {
  ...
  ## where AcP = AX*(I - BX'*(WBXp\WAXp))'
    ...
    # Sigma.X <- WBXp %*% Sigma.cP %*% t(WBXp)
    # Sigma.X <- (pseudoinverse(t(WBXp)) %*% Sigma.cP )%*%pseudoinverse(WBXp)
...
############################################################
    ...
    WAXp <- W %*% t(AX)  # N*1
    WBXpinv <- pseudoinverse(WBXp) # N*N

    BcP <- t(WBXpinv) %*% BX # N*J
    AcP <- AX %*% t(diag(J) - t(BX)%*% pseudoinverse(WBXp) %*% W)  # 1*J
...
gaussian.loadings <- function(maturities, K0d, K1d, H0d, rho0d, rho1d, timestep=1) {
    ...
    for (i in 1:maturities[M]) {
        ...
        #Atemp <- Atemp + K0dp%*%Btemp +.5%*%t(Btemp)
...
jsz.loadings.rho0cP <- function(W, K1Q.X, rho0.cP, Sigma.cP, mats, dt) {
    ...
    loads.X.prelim <- gaussian.loadings(mats.periods, K0Q.X, K1Q.X[1:N,1:N], matrix(0, N, N), rho0d*dt, rho1d*dt, dt)
    
    # print("GAUSSIAN LOADINGS!!!")
    # print(loads.X.prelim)
    # print(mats.periods)
    # print(K0Q.X)
    # print(K1Q.X)
    # print(rho0d*dt)
    ...
    if (any(is.na(WBXp))){
      # browser()
      loads.X.prelim <- gaussian.loadings(mats.periods, K0Q.X, K1Q.X[1:N,1:N], matrix(0, N, N), rho0d*dt, rho1d*dt, dt)
      test <- jszAdjustK1QX(K1Q.X_backup)
    }
    # browser()
    #Sigma.X <- solve(WBXp, Sigma.cP/(10^20)) %*% solve(t(WBXp)) # (W*BX')\Sigma.cP/(BX*W');
    # Sigma.X <- solve(WBXp, Sigma.cP %*% solve(t(WBXp)) # (W*BX')\Sigma.cP/(BX*W');
    Sigma.X <- pseudoinverse(WBXp) %*% Sigma.cP  %*% t(pseudoinverse(WBXp))
    ## Now with Sigma_X in hand, compute loadings for AX
    loads.X <- gaussian.loadings(mats.periods, K0Q.X, K1Q.X, Sigma.X, rho0d*dt, rho1d[1:N]*dt, dt)
    AX1 <- loads.X$A  # 1*J
    ...
    # browser()
    a0 <- matrix(1,1,N)%*%pseudoinverse(WBXp) %*%  (W%*%t(alpha0.X));
    a1 <- matrix(1,1,N)%*%pseudoinverse(WBXp) %*% (W%*%t(alpha1.X));
    kinfQ <- as.numeric(-(rho0.cP + a1)/a0);  ## -a0*kinf - a1 = rho0_cP
    K0Q.X[m1] <- kinfQ;

    AX <- alpha0.X*kinfQ + alpha1.X
    ## AcP = alpha0.cP*kinfQ + alpha1.cP
    alpha0.cP = t((diag(J) - (t(BX)%*%pseudoinverse(WBXp))%*%W)%*%t(alpha0.X));
    alpha1.cP = t((diag(J) - (t(BX)%*%pseudoinverse(WBXp))%*%W)%*%t(alpha1.X));
    ## could now get AcP using these -> check consistency with below
    
    # print("LOADING RHO0CP")
    # print(diag(J))
    # print(t(BX))
    # print(t(alpha0.X))
    # print(t(alpha1.X))
