getParameterNames <- function(N=3, include.P=TRUE, include.ev=TRUE) {
    param.names <- c('kinfQ',
                     ...
                     sapply( rep((1:N),(N+2))*10 + as.numeric(gl((N+2), N)), function(x) paste('Lam1_', as.character(x), sep='')),
                     sapply(1:((N+2)*(N+3)/2), function(x) paste('Sigma_', as.character(x), sep='')))
    if (include.P)
...
getAllSimResultsPersistence <- function(filenames) {
  ...
  for (filename in filenames) {
    ...
    for (j in 1:n) {
      ...
      pars.smpl <- getMCMCpars(kinfQ.i[ind], lamQ.i[ind,], lambda.i[ind,], Omega.i[ind,], Phi[ind,], Mu.i[ind,], mats, W, flag.mom=T, flag.rn=T, flag.forw=F)
      browser()
      col <- 0
      for (fn in pers.fns) {
        col <- col+1
...
printParameterEstimates <- function(filename, to.latex=FALSE) {
  ...
  Sigma.i <- t(apply( Omega.i, 1, function(z) as.numeric(t(chol(matrix(z, (N+2), (N+2))))) ))
  # browser()
  pars <- list(kinfQ = mean(kinfQ.i[mcmc.ind]),
               lamQ = colMeans(lamQ.i[mcmc.ind,])+1,
               lambda = colMeans(lambda.i[mcmc.ind,]),
               Sigma = matrix( colMeans(Sigma.i[mcmc.ind,]), (N+2), (N+2) ),
  ...
  pars.sd <- list(kinfQ = sd(kinfQ.i[mcmc.ind]),
                  ...
                  Sigma = matrix( apply(Sigma.i[mcmc.ind,], 2, sd), (N+2), (N+2)),
                  sigma.e = sd(sqrt(sige2.i[mcmc.ind])))
  
  ## credibility intervals
  q.p <- c(0.025, 0.975)
  # browser()
  lower.tri.ind <- as.numeric(which(lower.tri(matrix(0,(N+2),(N+2)), diag=T)))
  ...
  no.pars <- 1+N+N+(N*(N+2))+(N+2)*(N+3)/2+1
  ...
  ind.Sigma <- (max(ind.lambda)+1):(max(ind.lambda)+((N+3)*(N+2)/2))
  ## posterior mean
  ...
  tbl.sig <- matrix(0, 24, 12)
  col.alpha <- N+N+5
  col.ineff <- col.alpha+1
  col.prior <- 1:(N+2)
  col.post <- (N+3):(2*N+4)
  row.lam1 <- seq(6+1, 6+N*2, 2)
  row.Sigma <- seq(max(row.lam1+2), max(row.lam1)+5+2*N, 2)
  ...
  ## lamQ
  tbl[3,col.prior] <- c(rep((priors$lamQ.max+priors$lamQ.min)/2+1,N), c("-", "-"))
  tbl[4,col.prior] <- c(rep(sqrt(1/12*(priors$lamQ.max-priors$lamQ.min)^2),N), c("-", "-"))
  tbl[3,col.post] <- c(pars$lamQ, "-", "-")
  tbl.sig[3, col.post] <- c((apply(pars.q$lamQ, 2, prod)>0), FALSE, FALSE)
  tbl[4,col.post] <- c(pars.sd$lamQ, "-", "-")
  tbl[3,col.alpha] <- mean(alpha.lamQ[mcmc.ind])*100
  tbl[3,col.ineff] <- mean(apply(lamQ.i[mcmc.ind,], 2, ineff))
  ## lam0
  tbl[5,col.prior] <- 0
  tbl[6,col.prior] <- c(priors$lambda.sd[1:N]*1200, "-", "-")
  tbl[5,col.post] <- c(pars$lambda[1:N]*1200, "-", "-")
  tbl.sig[5,col.post] <- c(apply(pars.q$lambda[,1:N], 2, prod)>0, FALSE, FALSE)
  tbl[6,col.post] <- c(pars.sd$lambda[1:N]*1200, "-", "-")
  tbl[5,col.ineff] <- mean(apply(lambda.i[mcmc.ind,1:N], 2, ineff))
  ## lam1
  tbl[row.lam1,col.prior] <- 0
  tbl[row.lam1+1,col.prior] <- tail(priors$lambda.sd, N*(N+2))
  tbl[row.lam1,col.post] <- tail(pars$lambda, N*(N+2))
  tbl.sig[row.lam1,col.post] <- apply(pars.q$lambda[,(N+1):(N*(N+3))], 2, prod)>0
  tbl[row.lam1+1,col.post] <- tail(pars.sd$lambda, N*(N+2))
  tbl[min(row.lam1),col.ineff] <- mean(apply(lambda.i[mcmc.ind,(N+1):(N*(N+3))], 2, ineff))
  ## Sigma
  tbl[row.Sigma,col.post] <- pars$Sigma*1200
  Sigma.sig <- matrix(0, (N+2), (N+2))
  ...
  # tbl[is.na(tbl)] <- -9999999
  # print(round(tbl, digi=4))
  ## print(tbl.sig)
  # browser()
  tbl[is.na(tbl)] <- "-"
  tbl <- apply(tbl,2, as.numeric)
  ## latex table
  # browser()
  if (to.latex) {
    ...
    cat("\\begin{tabular}{lcccccccccccc} \\hline \\hline \n")
...
getMCMCpars <- function(kinfQ.i, lamQ.i, lambda.i, Omega.i, Phi.i, Mu.i, mats, W, flag.mom=T, flag.rn=T, flag.forw=T) {
  ...
  for (i in 1:M) {
    ...
    pars[[i]] <- list(kinfQ=kinfQ.i[i],
                      lamQ=lamQ.i[i,],
                      lambda=lambda.i[i,],
                      Omega=matrix(Omega.i[i,], (N+2), (N+2)),
                      Phi = matrix(Phi.i[i,], (N+2), (N+2)),
                      mu = Mu.i[i,])
    pars[[i]]$Sigma <-  t(chol(pars[[i]]$Omega))
    loads <- jsz.loadings(W, diag(lamQ.i[i,])[1:N,1:N], kinfQ.i[i], pars[[i]]$Omega[1:N,1:N], mats, dt=1)
    ...
    pars[[i]]$Lam1 <- matrix( tail(pars[[i]]$lambda, -N), N, (N+2))
    if (flag.mom) {
      ## unconditional moments
      pars[[i]]$EcP <- as.numeric(solve(diag(N) - pars[[i]]$Phi[1:N,1:N]) %*% pars[[i]]$mu[1:N])
      pars[[i]]$VarcP <- matrix( solve(diag(N^2) - kronecker(pars[[i]]$Phi[1:N,1:N], pars[[i]]$Phi[1:N,1:N]))%*%as.numeric(pars[[i]]$Omega[1:N,1:N]), N, N)
      pars[[i]]$VardcP <- (diag(N) - pars[[i]]$Phi[1:N,1:N]) %*% pars[[i]]$VarcP %*% t(diag(N) - pars[[i]]$Phi[1:N,1:N]) + pars[[i]]$Omega[1:N,1:N]
    }
    if (flag.rn) {
      ## loadings risk-neutral yields
      loads.rn <- gaussian.loadings(mats, pars[[i]]$mu[1:N], pars[[i]]$Phi[1:N,1:N]-diag(N), pars[[i]]$Omega[1:N,1:N], pars[[i]]$rho0, pars[[i]]$rho1)
    ...
    if (flag.forw) {
      ...
      H <- max(mats)
      # browser()
      loadsf <- getForwLoads(1:H, pars[[i]]$muQ, pars[[i]]$PhiQ, pars[[i]]$Omega[1:N,1:N], pars[[i]]$rho0, pars[[i]]$rho1)
      # browser()
      loadsfrn <- getForwLoads(1:H, pars[[i]]$mu[1:N], pars[[i]]$Phi[1:N,1:N], pars[[i]]$Omega[1:N,1:N], pars[[i]]$rho0, pars[[i]]$rho1)
...
getPostMeans <- function(pars.smpl) {
  ...
  return(list(kinfQ = mean(sapply(pars.smpl, function(z) z$kinfQ)),
              ...
              Omega = matrix(rowMeans(sapply(pars.smpl, function(z) z$Omega)), (N+2), (N+2)),
              Sigma = matrix(rowMeans(sapply(pars.smpl, function(z) z$Sigma)), (N+2), (N+2)),
              mu = rowMeans(sapply(pars.smpl, function(z) z$mu)),
              Phi = matrix(rowMeans(sapply(pars.smpl, function(z) z$Phi)), (N+2), (N+2)),
...
printGammaStats <- function(filenames, to.latex=FALSE) {
  ...
  out.matrix <- matrix(NA, N+(N*(N+2)), 3*length(filenames))
...
printModels <- function(filenames, all.models, to.latex=FALSE) {
  ...
  #print(tbl)
    ...
    for (i in 2:length(filenames)) {
        ...
        rel.freq <- sapply(index, function(model) sum(model.i==model)/length(mcmc.ind))
        #browser()
        print(rel.freq)
        print(filenames[i])
    ...
    if (to.latex) {
        ...
        cat("models ")
        for (i in 1:N)
            cat("& \\multicolumn{2}{c|}{", visited[i], "/", 2^(N+N*(N+2)),"}")
        cat("\\\\ \n")
        cat("visited ")
        for (i in 1:N)
            cat("& \\multicolumn{2}{c|}{", round(100*visited[i]/2^(N+N*(N+2)), digi=1), "\\%}")
        cat("\\\\ \n")
        cat("\\hline \n")
        sink()
    }
...
loadModel <- function(name, filename) {
    ...
    model <- list(
        ...
        Omega.i[mcmc.ind,], Phi.i[mcmc.ind,], Mu.i[mcmc.ind,], mats, W),
        W = W, N = N, M = M, mats = mats, J = length(mats),
        cP = cbind(Y %*% t(W),MF), T = length(dates))
    model <- within(model, {
        ## posterior means of parameters
        pars <- getPostMeans(pars.smpl)
        ## model-implied short rate
        # browser()
        r <- rep(1,T)*pars$rho0 + cP[,1:N] %*% pars$rho1
        ## UNCONDITIONAL MOMENTS
        ## (1) moments at point estimates
        # browser()
        # EcP <- as.numeric(solve(cbind(diag(N), matrix(rep(0,2*N),nrow =N)) - pars$Phi[1:N,1:(N+2)]) %*% pars$mu[1:(N+2)])
        EcP <- as.numeric(solve(diag(N+2) - pars$Phi)[1:N,1:(N+2)] %*% pars$mu[1:(N+2)])
        VarcP <- matrix( solve(diag(N^2) - kronecker(pars$Phi[1:N,1:N], pars$Phi[1:N,1:N]))%*%as.numeric(pars$Omega[1:N,1:N]), N, N)
        ...
        # browser()
        Yhat <- rep(1,T)%*%pars$A + cP[,1:N]%*%pars$B
        Yrn <-  rep(1,T)%*%pars$Arn + cP[,1:N]%*%pars$Brn
        Ytp <- Yhat - Yrn
        ## forward rates and term premia
        H <- max(mats)
        fhat <- rep(1,T) %*% pars$Af + cP[,1:N] %*% pars$Bf
        frn <- rep(1,T) %*% pars$Afrn + cP[,1:N] %*% pars$Bfrn
        ftp <- fhat-frn
    })
    return(model)
}
...
printPersVol <- function(models, to.latex = FALSE) {
    ...
    stat.fns <- list(
        ...
        function(pars) max(abs(eigen(pars$Phi[1:N,1:N])$values)),
        function(pars) {
            Bf <- 2*pars$B[,mats==120] - pars$B[,mats==60]
...
plotExpTP <- function(models, to.file=FALSE, mat, name) {
    ...
    if (to.file) {
        filename <- paste("figures/exp_tp_MEF", name,  ".eps", sep = "") 
    ...
    ## top panel
    par(mar = c(4,4,2,1)+.1)
    plot(actual, type="l", ylab="Percent Yield", ylim=yrange, xlab = "Time", col=colors[1], lwd=lwds[1], lty=ltys[1])
    ...
    fillTable <- function(tbl) {
        ...
        for (m in models) {
            ...
            rn.from.smpl <- 1200*sapply(m$pars.smpl, function(pars)
                                        pars$Arn[,j2]+cP.from[1:N]%*%pars$Brn[,j2])
            rn.to.smpl <- 1200*sapply(m$pars.smpl, function(pars)
                                      pars$Arn[,j2]+cP.to[1:N]%*%pars$Brn[,j2])
            ## ## forward rates
            ## rn.from.smpl <- 1200*sapply(m$pars.smpl, function(pars)
...
calculateReturns <- function(model) {
  return(within(model, {
    ...
    # browser()
    Sig.lambda.t <- rep(1,T)%o%pars$Lam0[1:N] + cP[,1:N] %*% t(pars$Lam1) ## lambda_t
    ...
    pars.smpl <- lapply(pars.smpl, function(pars) {
      loads.nm1 <- gaussian.loadings(mats-1, pars$muQ, pars$PhiQ-diag(N), pars$Omega[1:N,1:N], pars$rho0, pars$rho1)
      pars$Anm1 <- loads.nm1$A; pars$Bnm1 <- loads.nm1$B
      loads.nmh <- gaussian.loadings(mats.nmh, pars$muQ, pars$PhiQ-diag(N), pars$Omega[1:N,1:N], pars$rho0, pars$rho1)
      pars$Anmh <- loads.nmh$A; pars$Bnmh <- loads.nmh$B
      loads.h <- gaussian.loadings(h, pars$muQ, pars$PhiQ-diag(N), pars$Omega[1:N,1:N], pars$rho0, pars$rho1)
    ...
    Erx.mean.smpl <- sapply(pars.smpl, function(pars) {
      ...
      for (n in mats) {
        # browser()
        cBnm1 <- as.numeric(-(n-1)*pars$Bnm1[,mats==n])
        Erx.mean[mats==n] <- -.5*cBnm1 %*% pars$Omega[1:N,1:N] %*% cBnm1 + t(cBnm1) %*% (pars$Lam0 + pars$Lam1[1:N,1:3] %*% pars$EcP)
      }
      return(Erx.mean)
    })
    ## calculate realizes and expected excess returns for h-month holding period
    # browser()
    yhhat <- rep(1,T)*pars$Ah + cP[,1:N]%*%pars$Bh # need fitted yield with maturity h
    cAh <- -h*pars$Ah; cBh <- as.numeric(-h*pars$Bh) # bond price loadings: cA.n <- -n*A.n
    Phi.h <- matrix.power(pars$Phi, h)
    # browser()
    for (i in 1:length(mats[mats>h])) {
      n <- mats[mats>h][i]
      n_ind <- i + 8
      # browser()
      rx.h[,mats==n] <- - (n-h)*Yall[(1+h):T, n_ind-1] + n*Yall[1:(T-h), n_ind] - h*Yall[1:(T-h), 1]
      ...
      Erx.h[,mats==n] <- rep(cAnmh - cAn + cAh + EcP %*% (cBnmh - cBn + cBh), T-h) +
        t(t(cP[1:(T-h),1:N])-EcP) %*% t(cBnmh%*%Phi.h[1:N,1:N] - cBn + cBh)
    }
    # browser()
...
getPredPop <- function(Bh, Bn, Bnmh, Phi, Omega, h) {
  ...
  R2 <- numeric(J)
  cBh <- as.numeric(-h*Bh)
  VarcP <- matrix( solve(diag(N^2) - kronecker(Phi[1:N,1:N], Phi[1:N,1:N]))%*%as.numeric(Omega[1:N,1:N]), N, N)
  ...
  for (i in 1:length(mats[mats>h])) {
    ...
    Phi.i <- diag(N)
    for (i in 0:(h-1)) {
      tmpsum <- tmpsum + Phi.i %*% Omega[1:N,1:N] %*% t(Phi.i)
      Phi.i <- Phi[1:N,1:N] %*% Phi.i
...
getPredSmallSample <- function(An, Anmh, Bn, Bnmh, mu, Phi, Sigma, h, T, M=1000) {
  ...
  EcP <- as.numeric(solve(diag(N+2) - Phi)[1:N,] %*% mu) # unconditional mean
  VarcP <- matrix( solve(diag(N^2) - kronecker(Phi[1:N,1:N], Phi[1:N,1:N]))%*%as.numeric(Sigma[1:N,1:N] %*% t(Sigma[1:N,1:N])), N, N)
  ...
  for (b in 1:M) {
    ...
    # browser()
    for (t in 2:T)
      cPsim[t,] <- mu[1:N] + Phi[1:N,1:N] %*% cPsim[t-1,] + Sigma[1:N,1:N] %*% rnorm(N)
...
extremeScenarios <- function(filename, H, quant, dim, mat_scen) {
  ## generate extreme scenarios for the yield curve in a year
  cat("determining scenario for model: ", filename, "\n")
  set.seed(4493)
  load(filename)
  library(MASS)
  ...
  Y.fore = matrix(NA, (M-(start-1)), length(mats))
  Y.fore.indiv <- array(NA, c((M-(start-1)), length(mats), H))
  aver_score = matrix(NA, (M-(start-1)),1)
  for (m in start:M){
    if( m %% 1000 == 0) {
      cat("*** iteration ",m,"\n")
    }
    # lam0 <- lambda.i[m,1:N]
    # lam1 <- lambda.i[m,(N+1):length(lambda.i[1,])]
    lambda <- lambda.i[m,]
    lamQ <- lamQ.i[m,1:N]
    kinfQ <- kinfQ.i[m]
    Omega <- matrix(Omega.i[m,], nrow = dim)
    Sige2 <- sige2.i[m]
    loads <- jsz.loadings(W, diag(lamQ), kinfQ, Omega[1:N,1:N], mats, dt =1)
    Pdyn <- getPdyn(loads, lambda)
    if (dim > 3){
      Mu.full <- c(Pdyn$mu, Mu.i[m,][(N+1):(N+2)])
      Phi.full <- rbind(Pdyn$Phi, matrix(Phi.i[m,], nrow = 5)[(N+1):(N+2),])
      cP.fore <- matrix(0, (N+2), (H+1))
      cP.fore[,1] <- ZP[length(ZP[,1]),]
      for (t in 1:H){
        cP.fore[,t+1] <- Mu.full + Phi.full %*% cP.fore[,t] + mvrnorm(mu = rep(0, (N+2)), Sigma = Omega)
        Y.fore.indiv[m-(start-1),,t] <- loads$AcP + t(t(loads$BcP) %*% cP.fore[1:N,(t+1)]) + mvrnorm(mu = rep(0, length(mats)), Sigma = diag(Sige2, length(mats)))
      }
      # browser()
      Y.fore[m-(start-1),] <- loads$AcP + t(t(loads$BcP) %*% cP.fore[1:N,(H+1)]) + mvrnorm(mu = rep(0, length(mats)), Sigma = diag(Sige2, length(mats)))
      # browser()
    }else{
      Mu.full <-Pdyn$mu
      Phi.full <- Pdyn$Phi[1:dim,1:dim]
      cP.fore <- matrix(0, dim, (H+1))
      # browser()
      cP.fore[,1] <- cP[length(cP[,1]),]
      for (t in 1:H){
        cP.fore[,t+1] <- Mu.full + Phi.full %*% cP.fore[,t] + mvrnorm(mu = rep(0, dim), Sigma = Omega)
        Y.fore.indiv[m-(start-1),,t] <- loads$AcP + t(t(loads$BcP) %*% cP.fore[1:N,(t+1)]) + mvrnorm(mu = rep(0, length(mats)), Sigma = diag(Sige2, length(mats)))
      }
      # browser()
      Y.fore[m-(start-1),] <- loads$AcP + t(t(loads$BcP) %*% cP.fore[1:N,(H+1)]) + mvrnorm(mu = rep(0, length(mats)), Sigma = diag(Sige2, length(mats)))
    }
    aver_score[m-(start-1)] <- sum(Y.fore[m-(start-1),]) 
  } 
  # browser()
  scenario.indiv <- matrix(NA, length(mats), H)
  for (i in 1:H){
    scenario.indiv[,i] <- apply(Y.fore.indiv[,,i], MARGIN = 2, FUN = quantile, probs = quant)
    
  }
  scenario <- apply(Y.fore, MARGIN = 2, FUN = quantile, probs = quant) 
  score_index <- which.min(abs(Y.fore[,1]-quantile(Y.fore[,1], quant)))
  score_index_long <- which.min(abs(Y.fore[,7]-quantile(Y.fore[,7], quant)))
  
  score_index_av <- which.min(abs(aver_score-quantile(aver_score, quant)))
  
  # browser()
  score_scen <- Y.fore[score_index,]
  score_scen_long <- Y.fore[score_index_long,]
  score_scen_aver <- Y.fore[score_index_av,]
                           
  par_shock <- quantile(aver_score, quant)
  return(list(scenario = scenario, scenario.indiv = scenario.indiv, score_scen = score_scen, score_scen_long = score_scen_long, score_scen_aver = score_scen_aver, par_shock = par_shock))
}
